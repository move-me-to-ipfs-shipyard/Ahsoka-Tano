...
(defonce program-data-dirpath (or
                               (some-> (System/getenv "Ahsoka-Tano_PATH")
                                       (.replaceFirst "^~" (System/getProperty "user.home")))
                               (.getCanonicalPath ^File (clojure.java.io/file (System/getProperty "user.home") "Ahsoka-Tano"))))

(defonce state-file-filepath (.getCanonicalPath ^File (clojure.java.io/file program-data-dirpath "Ahsoka-Tano.edn")))
...
(def ^:const jframe-title "i've seen what such feelings can do to a fully trained jedi knight")
...
(defn settings-process
  ...
  (let [jscroll-pane (JScrollPane.)
    ...
    (doto jpanel-tab
      ...
      (.add jcheckbox-apricotseed "cell 1 0"))
...
(defn -main
  [& args]
  (println "surrender - or face the consequences")
